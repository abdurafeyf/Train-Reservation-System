...
class BookingDetails(QtWidgets.QWidget):
    ...
    def __init__(self, parent, currentBooking):
        ...
        self.seatPreferencecomboBox.currentTextChanged.connect(self.showing_windowAisle)
        ...
        if currentBooking:
            ...
            self.departureCityComboBox.setCurrentText(currentBooking["Departure City"])
            self.destinationCityComboBox.setCurrentText(currentBooking["Destination City"])
            ...
            self.seatPreferencecomboBox.setCurrentText(currentBooking["Seat Preference"])
            
            self.seatBerthNumbercomboBox.setCurrentText(str(currentBooking["Seat No"]))
            
            self.ageLine.setText(str(currentBooking["Passenger Age"]))
            self.WindowAislecomboBox.setCurrentText(currentBooking["Window/aisle"])
...
class SuccessfulBooking(QtWidgets.QWidget):
    ...
    def __init__(self, parent, currentBooking):
        ...
        self.WnidowOrAisleLabel.setText(BookingDetails.bookingInfo["Seat Preference"])
...
class PreviousBookings(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.UpdateBookingButton.clicked.connect(self.on_clickUpdateBookingButton)
        self.CancelBookingButton.clicked.connect(self.on_clickCancelBookingButton)
        ...
        for userdata in list_of_tickets_of_signed_users:
            ...
            self.tableWidget.setItem(rowPosition, 0, QTableWidgetItem(str(userdata["Booking Id"])))
            self.tableWidget.setItem(rowPosition, 1, QTableWidgetItem(userdata["Passenger Name"]))
            self.tableWidget.setItem(rowPosition, 2, QTableWidgetItem(userdata["Departure Date"]))
            self.tableWidget.setItem(rowPosition, 3, QTableWidgetItem(userdata["Departure City"]))
            self.tableWidget.setItem(rowPosition, 4, QTableWidgetItem(userdata["Destination City"]))
            self.tableWidget.setItem(rowPosition, 5, QTableWidgetItem(userdata["Seat Preference"]))
            # self.tableWidget.setItem(rowPosition, 6, QTableWidgetItem(userdata["Seat No"]))
            self.tableWidget.setItem(rowPosition, 6, QTableWidgetItem(str(userdata["Fare"])))
            

    def on_click_updateButton(self):
        self.AskBookingIdLabel.setText("Enter Booking ID for updating that booking ")
        self.BookingIdlineEdit.show()
        self.UpdateBookingButton.show()
        self.CancelBookingButton.setHidden(True)

    def on_click_cancelButton(self):
        self.AskBookingIdLabel.setText("Enter Booking ID for cancelling that booking ")
        self.BookingIdlineEdit.show()
    ...
    def on_clickCancelBookingButton(self):
        ...
        # pass query to delete the bookings on that booking id 
...
class CancelledBookings(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.ShowCancelDetailsLabel.setText(f"Booking ID {id} on the name of {name} and \nCNIC {CNIC} has been Cancelled")
